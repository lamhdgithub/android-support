...
import android.support.core.navigation.*
...
class FragmentNavigator(
    ...
    override fun navigateUp(result: BundleArgument?): Boolean {
    ...
    private fun Fragment.notifyResultIfNeeded(result: BundleArgument?) {
        if (this is FragmentResultCallback) {
            if (result == null) return
            val liveData = this.viewLifecycleOwnerLiveData
            liveData.observe(this, OnceTimeSubscribeStartLifecycleObserver(liveData) {
                this.onFragmentResult(result)
            })
        }
    }

    private class OnceTimeSubscribeStartLifecycleObserver(
        private val liveData: LiveData<LifecycleOwner>,
